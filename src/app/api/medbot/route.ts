...
export async function POST(req: Request) {
  try {
    const { message, isFirstMessage, isPageReload } = await req.json();
    const headersList = await headers();
    const currentSessionId = headersList.get('x-session-id') || '';
    const model = getMedbotModel();

    // Reset session if page is reloaded, ID changed, or doesn't match
    if (isPageReload || currentSessionId !== chatSession.sessionId) {
      chatSession = {
        instance: null,
        hasInitialContext: false,
